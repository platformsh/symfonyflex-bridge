...
function mapPlatformShEnvironment() : void
{
    ...
    // Map original host. Applicable for non-master Platform.sh environments.
    mapPlatformOriginalHost();
}
 ...
 * Sets original host
 *
 * Autogenerated host names are used on non-master Platform.sh environments.
 * `HTTP_X_ORIGINAL_ROUTE` variable is used to extract original host
 * and set it as `X_FORWARDED_HOST` server variable. Which is respected by Symfony.
 */
function mapPlatformOriginalHost(): void
{
    $originalRoute = getenv('HTTP_X_ORIGINAL_ROUTE');
    if ($originalRoute === false) {
        return;
    }

    $originalHost = parse_url($originalRoute, PHP_URL_HOST);
    if ($originalHost) {
        setEnvVar('X_FORWARDED_HOST', $originalHost);
    }
}